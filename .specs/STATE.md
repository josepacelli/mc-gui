...
- **Sessão atual (2026-09-11), tudo em `main`, sem branch separada**:
  1. **Bug fix**: Insert/`+` desmarcava o item errado e não sincronizava com clique do mouse — causa raiz era `selection` (binding nativo do `List`, usado como cursor) fazendo dupla função de "marcados". Separado em `selection` (cursor, só highlight) e `markedIDs` (marca real); `FileRow` ganhou destaque visual próprio pra marcados (`597d9f2`).
  2. **Feature nova `Ir para Pasta`**: duplo-clique na barra de caminho (ou botão à direita dela) abre diálogo com campo de texto + árvore de diretórios lazy-load, expande até a pasta atual, clique na árvore atualiza o campo e destaca a linha com fundo cheio (`GoToFolderDialog`/`GoToFolderDialogViewModel`/`DirectoryTreeNodeViewModel`, mesmo commit `597d9f2`, sem spec formal — pedido direto, escopo pequeno).
  3. Velocidade (B/s, MB/s, GB/s) adicionada ao `ProgressDialog` (não commitado ainda — ver Uncommitted files abaixo).
  4. Release `v1.0.1` taggeada e pushed, disparando o workflow de build/release.
  5. **Feature `drag-drop-copy`** (spec-driven completo: specify → design → tasks → execute → verify): arrastar item/conjunto marcado de um painel pro outro abre o mesmo `CopyMoveDialog` do F5 (nunca copia direto no drop). `Transferable`/`.draggable`/`.dropDestination` nativo do SwiftUI, payload `DraggedFileURLs` (paths + `sourcePanelID` pra ignorar drop no mesmo painel). **DONE — Verifier PASS** (iteração 2/2 do fix→reverify; iteração 1 achou um bug real de processo: `git add Tests/...` com T maiúsculo não staga nada nesse filesystem case-insensitive, path real é `tests/` minúsculo — 4 commits ficaram sem os testes no histórico até o Verifier pegar; corrigido em `9ce11da`/`0f25e9a`). 344 testes, sensor 2/2 mortos. Único item aberto: DND-11 (UAT manual — sem ferramenta de automação de GUI nativa disponível). Relatório: `.specs/features/drag-drop-copy/validation.md`.
  6. **Feature `context-menu-actions`** (spec-driven completo, executado via 2 lotes de sub-agente): menu de contexto real (Abrir/Selecionar-Desselecionar/Zipar/Editar/Apagar/Mostrar Informações) substituindo o placeholder. Abrir/Selecionar/Editar/Apagar só reusam lógica existente (`activate`/`toggleSelection`/`onEditFile`/`makeDeleteDialog`). Zip é capacidade nova: `FileSystemService.zip()` roda `/usr/bin/zip -r -X -y` via `Process` (array de argumentos, nunca shell string — novo `AD-006`), escreve em nome temporário e só move pro final se sair com sucesso (nunca deixa `.zip` corrompido). Mostrar Informações é `InfoDialogViewModel` novo, tamanho de pasta calculado recursivo reaproveitando `listDirectory` (sem novo método no protocolo). **DONE — Verifier PASS**, 359 testes, sensor 4/4 mortos. Gap não-bloqueante: CTXM-17 (target de symlink no diálogo de info) sem teste dedicado (lição L-021, candidate). Relatório: `.specs/features/context-menu-actions/validation.md`.
- **Lições candidatas novas desta sessão** (L-016 a L-021 em `.specs/lessons.json`, nenhuma ainda `confirmed`): disciplina de commit de testes, extrair lógica de orquestração async em função estática testável, não testar semântica de value-type da própria linguagem, gotcha de `git add` case-insensitive, campo condicional de view-model merece teste próprio mesmo com view não-testável.
- **Next step**: pedir ao usuário pra rodar o app (`swift run MCGuiApp`) e confirmar manualmente: drag-and-drop entre painéis (incluindo soltar fora de qualquer painel e clique/duplo-clique/botão-direito continuarem funcionando na mesma linha), e o menu de contexto completo (6 itens, Zip, Mostrar Informações). Depois: decidir se comita a mudança pendente de velocidade no `ProgressDialog` (item 3 acima) e se dá `git push`.
- **Blockers**: none.
- **Uncommitted files**: `Sources/MCGuiUI/Views/ProgressDialog.swift` (velocidade B/s-MB/s-GB/s, item 3 acima — funcional, testado manualmente, só nunca commitado) e `packaging/build-macos.sh` (mudança pré-existente não-relacionada a esta sessão, nunca investigada/tocada).
- **Branch**: `main`, local à frente do `origin/main` (não confirmado se pushed até o HEAD atual — checar antes de assumir).

---

### Handoff anterior (sessão de `convert-to-swift-swiftui`, arquivado)
