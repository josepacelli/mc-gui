...
- **Confirmado pelo usuário (DMG instalado)**: clique simples/duplo-clique (item 5), F1 Help, F2 User Menu. **Ainda não confirmado**: cópia/move async (item 12, janela de progresso não-modal).
- **Next step**: pedir confirmação da cópia async (item 12) quando o usuário testar uma cópia/move de verdade. Gaps grandes que sobraram, não iniciados: paginação de pastas com 10k+ arquivos (decisão explícita do usuário de deixar pra depois) e as features do mc original ainda sem nenhum código (Find File, Tree, Diff Viewer, Chmod/Chown/Chattr, Subshell, VFS/FTP, fila de operações em background, tela de configurações, Windows/Linux).
- **Blockers**: none.
