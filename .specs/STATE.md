...
- **Feature `viewer-f3`**: **PARCIAL (6/7 tasks)** — T1-T4, T6, T7 done. Text mode viewer with wrap, scroll, keyboard nav; incremental search (Ctrl+F); F3 key binding; File > View menu; search bar; F1-F10 toolbar. **T5 (Hex mode) SKIPPED - will return later.** 266 tests pass.
- **Next step**: Return to T5 (Hex mode toggle) for viewer-f3. Then continue specifying remaining MC features: Editor (F4), VFS (FTP/SFTP/archives), Hotlist/Bookmarks (Ctrl+\), User Menu (F2), Directory Tree, Find File (Alt+F7), Background Operations, Chmod/Chown/Chattr, Help (F1), Diff Viewer, Subshell/Command Line, Configuration/Setup, Windows/Linux installers.
- **Blockers**: none
- **Uncommitted files**: ViewerTemplateSelector.cs (not used, needs removal), ViewerWindow.axaml (needs update for hex mode), STATE.md.
- **Branch**: main, 14 commits locais à frente de origin — pedir go-ahead antes de push.
